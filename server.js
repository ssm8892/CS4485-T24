...
app.post('/tutor-login', async (req, res) => {
  ...
  if (dbResult.length > 0) {
    ...
    const queryAppt = `select * from appointments where tutor_name = '${tempFullName}';`;
    const dbAppt = await executeRows(queryAppt);

    for (let i = 0; i < dbAppt.length; i++) {
      // Dictionary of tutor info
      const apptDict = {
        ID: dbAppt[i]['appontment_id'],
        date: dbAppt[i]['written_date'],
        time: dbAppt[i]['written_time'],
...
app.post('/login', async (req, res) => {
  ...
  if (dbResult.length > 0) {
    ...
    // Get tutor appointments
    const tempFullName = global.firstName + " " + global.lastName;
    const queryAppt = `select * from appointments where student_name = '${tempFullName}';`;
    const dbAppt = await executeRows(queryAppt);
...
app.post('/favorites', async(req, res) => {
  ...
  const possibleFavorite = await executeRows(possibility);

  var existing = possibleFavorite.length;
  
  if (existing == 0) {
    const queryInsert = `insert into favorites (student_fname, student_lname, student_email, tutor_name) values('${global.firstName}', '${global.lastName}', '${global.email}', '${val}');`;
    
    // Execute query insert
    con.query(queryInsert, (err, rows) => {
      if (err)
        console.log("Error");
    });
  }
  else {
    const queryDelete = `delete from favorites where student_fname = '${global.firstName}' and student_lname = '${globalLastName}' and email = '${global.email}' and tutor_name = '${val}';`;

    // Execute query insert
    con.query(queryDelete, (err, rows) => {
      if (err)
        console.log("Error");
    });
  }
